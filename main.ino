 ...
 *   * Separate web pages for: GPS map, sensor data, manual test, and settings
 *   * NEO-6M GPS for Lat/Lon/Speed
 *   * Follows all pinout and logic from provided code
 *
 *   * Manual alert via BUTTON is DISABLED to prevent false triggering.
 *     Only web-based manual alert is possible (optional: can comment out web-based handler too).
 */
...
void loop() {
    ...
    // COMMENTED OUT: No manual alert trigger via button
    // if (!button && buttonPressStart > 0 && millis() - buttonPressStart < 2000) {
    //     if (!alertActive) manualAlert = true;
    //     buttonPressStart = 0;
    // }
    // Long hold (10s) = reset
...
void checkAndSendAlert() {
    ...
    if (!alertActive && (highG || flipped || manual)) {
        ...
        String mapsLink = (gpsIsValid && gpsLat != 0 && gpsLon != 0)
            ? "https://maps.google.com/?q=" + String(gpsLat,6) + "," + String(gpsLon,6)
            : "Unavailable";
...
void handleSendSMS() {
    ...
    String msg = String("CAR ACCIDENT ALERT!\n")
        + "Cause: Web Manual"
        + "\nG's: " + String(lastG,2)
        + "\nLocation: " + (gpsIsValid && gpsLat != 0 && gpsLon != 0 ? "https://maps.google.com/?q="+String(gpsLat,6)+","+String(gpsLon,6) : "Unavailable")
        + "\nState: " + lastOrientation;
